...
def get_user_info_for_gamemode(mlpyvrs, user, gmrating : GamemodeRating, gmrank : GamemodeRank, match = None):
    info = {}
    ...
    character_slug = user.get_top_ranked_character_in_gamemode(gmrating) if match is None else match.get_player_data_by_id(info["id"]).get_character_slug()
    char = get_character_from_slug(character_slug)
    info["ranked_win"] = user.get_wins_with_character(char)
    info["char"] = char.value["name"] 
    info["rating"] = round(user.get_character_rating(char, RatingKeys.Mean, gmrating), 0)
    info["rank"] = -1
